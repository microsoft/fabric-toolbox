        ...
        static public void WriteDataTableToParquet(DataTable table, string filePath)
        {
            try
            {
                ...
                for (int i = 0; i < table.Columns.Count; i++)
                {
                    ...
                    // Console.WriteLine($"CCC {column.ColumnName} {column.DataType}"); 
                }
                // Write Parquet file
                        ...
                        using (var rowGroupWriter = parquetWriter.AppendRowGroup())
                        {
                            ...
                            foreach (DataColumn column in table.Columns)
                            {
                                ...
                                try
                                {
                                    ...
                                    {
                                        int[] intArray = Array.ConvertAll(columnData, item => Convert.ToInt32(item));
                                ...
                                {
                                    Console.WriteLine($"WriteDataTableToParquet:loop" +
                                        $"-{ex.Message}");
                                }
                            }
                        }
            ...
            {
                Console.WriteLine($"WriteDataTableToParquet-{ex.Message}");
            }
        }
