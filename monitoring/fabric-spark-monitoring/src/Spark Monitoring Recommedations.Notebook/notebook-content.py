...
def get_cores_per_executor(metadata_df, app_id):
    """Extract cores per executor from metadata, with intelligent fallbacks"""
    cores_per_executor = 4  # Default fallback
    
    if metadata_df is not None:
        try:
            # Try to get from metadata DataFrame
            metadata_rows = metadata_df.filter(col("app_id") == app_id).collect()
            if metadata_rows:
                metadata_row = metadata_rows[0]
                if hasattr(metadata_row, "spark.executor.cores") and metadata_row["spark.executor.cores"] is not None:
                    cores_per_executor = int(metadata_row["spark.executor.cores"])
                elif hasattr(metadata_row, "isHighConcurrencyEnabled") and metadata_row["isHighConcurrencyEnabled"]:
                    # High concurrency clusters typically have 8 cores per executor
                    cores_per_executor = 8
        except Exception as e:
            print(f"⚠️ Could not extract cores from metadata for {app_id}: {e}, using default {cores_per_executor}")
...
def compute_advanced_executor_metrics(event_log_df):
    ...
    try:
        ...
        # Get cores per executor from metadata for more accurate analysis
        cores_per_executor = get_cores_per_executor(metadata_df, str(app_id))
        print(f"📊 Using {cores_per_executor} cores per executor for analysis")
        
        parallelism_score = min(1.0, task_count / max(1, executor_count * cores_per_executor))
...
def compute_advanced_stage_task_summary(event_log_df, metadata_df, app_id):
    ...
    # Extract cores per executor from metadata early in the analysis
    cores_per_executor = get_cores_per_executor(metadata_df, str(app_id))
    print(f"📊 Using {cores_per_executor} cores per executor (from metadata)")
    try:
        task_end_df = event_log_df.filter(col("properties.Event") == "SparkListenerTaskEnd")
...
def generate_enhanced_recommendations(app_id, app_duration_sec, driver_metrics, executor_metrics, workload_analysis, metadata_df, task_df):
    ...
    # Extract cores per executor from metadata
    cores_per_executor = 4  # default fallback
    try:
        if metadata_df is not None:
            metadata_row = metadata_df.filter(col("app_id") == app_id).first()
            if metadata_row and hasattr(metadata_row, "spark.executor.cores"):
                cores_per_executor = int(getattr(metadata_row, "spark.executor.cores", 4))
    except Exception as e:
        print(f"⚠️ Could not extract cores from metadata: {e}, using default 4")
...
def per_app_analyzer(pdf: pd.DataFrame) -> pd.DataFrame:
    ...
    if app_start_props or first_record:
        ...
        # Extract executor cores configuration
        cores_per_executor = None
        if spark_props:
            cores_prop = spark_props.get("spark.executor.cores")
            if cores_prop is not None:
                try:
                    cores_per_executor = int(cores_prop)
                except (ValueError, TypeError):
                    pass
        
        # Fallback to default if not specified (typical Fabric defaults: 4 for regular, 8 for High Concurrency)
        if cores_per_executor is None:
            # High concurrency clusters typically have more cores per executor
            is_hc = outer_metadata.get("isHighConcurrencyEnabled")
            if is_hc and str(is_hc).lower() == "true":
                cores_per_executor = 8  # Typical high concurrency default
            else:
                cores_per_executor = 4  # Typical standard default
        ...
        metadata_payload = {
            ...
            "spark.executor.cores": 4,  # Default assumption when metadata unavailable
        }
    ...
    # TODO: Get cores_per_executor from metadata context - for now using 4 as fallback
    cores_per_executor = 4  # Should be extracted from metadata in the calling function
    parallelism_score = min(1.0, (task_count / max(1, executor_count * cores_per_executor))) if executor_count > 0 else 0.0
    ...
    def classify_workload_type(features):
        ...
        # Get cores per executor for this analysis
        cores_per_executor = 4  # Will be passed from calling context in future refactor
        
        # Ideal: 2-4 tasks per executor core
        ideal_tasks_per_executor = cores_per_executor  # 1 task per core is ideal balance
        resource_balance = min(1.0, features['task_density'] / ideal_tasks_per_executor)
        ...
        elif features['parallelism_score'] > 0.6 and features['executor_efficiency'] > 0.4:
        ...
        elif resource_balance > 0.8:  # > 3.2 tasks per executor
            return 'overloaded', 0.8  # Can benefit from more executors
        elif resource_balance < 0.5:  # < 2 tasks per executor  
            return 'underutilized', 0.3  # Too many executors for available work
        else:
    ...
    if task_count == 0:
        ...
        driver_ratio = 0.20 + (0.5 - features['work_efficiency']) * 0.4
            ...
            # Task density should ideally be calculated using actual cores per executor
            driver_ratio = 0.15 + (1.0 - min(1.0, features['task_density'] / cores_per_executor)) * 0.15
        else:
    ...
    for m in multipliers:
        ...
        elif workload_type == 'overloaded':
            # Learned: Overloaded executors benefit from scaling up
            scaling_efficiency = 0.70 + (features['task_density'] / 4.0 * 0.15)
            coordination_overhead_factor = 0.5
        elif workload_type == 'underutilized':
            # Learned: Too many executors for available work - scaling up has limited benefit
            scaling_efficiency = 0.25 + (features['task_density'] / 4.0 * 0.2)
            coordination_overhead_factor = 1.2
        ...
        if input_data_skew > 10 and len(input_bytes_list_calc) > 100:
            if is_streaming_job:
                anti_patterns.append(
                    ...
                    f"(4) Add .repartition({max(cores_per_executor, executor_count*cores_per_executor)}) immediately after readStream, "
        ...
        if features['task_count'] > 500 and features['input_bytes_avg'] < 1024 * 1024:  # <1MB per task
            ...
            if is_streaming_job:
                ...
                anti_patterns.append(
                    ...
                    f"Overhead ratio: {overhead_ratio:.1f} tasks per MB. "
                    f"FIX: (1) Compact files before processing: df.coalesce(num_executors * {cores_per_executor}).write.save(), "
