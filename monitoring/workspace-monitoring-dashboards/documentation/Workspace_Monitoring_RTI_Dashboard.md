...
The Real-Time Dashboard in Microsoft Fabric enables users to configure live connections to Monitoring Eventhouse, providing up-to-the-moment visibility into workspace activity and operations. This repository hosts **Real-Time Dashboard** templates that you can connect directly to your Monitoring Eventhouse databases to visualize and monitor real-time data and insights.

With this dashboard, you can seamlessly track your workspace items, operations, visuals, and more, all within the Microsoft Fabric SaaS experience - enabling proactive monitoring and immediate response to
...
|Step|Description|
|---|---|
|1.|**Workspace** parameter: **Select** a connected workspace to set the scope of the report. <br> Also the overview page shows you the name of the selected workspace. |
|2.|**Other** parameters: Each dashboard page contains context specific parameters, which you can select <br> **UTC offset** parameter transfers all the time related insights to the selected time zone. <br> **Time range** parameter filters all the visuals on the page (maximum available interval: 30 days). |
...
|6.|Available log cards show you which workloads logs are available in the current time window for the selected workspace.|
...
|5.|Basic Insights about semantic models like "Operations by status over time" and "Operations by Application"|

![Screenshot](/monitoring/workspace-monitoring-dashboards/media/documentation/rti/fwm_rti_ap_2.png)

|Step|Description|
|---|---|
|Info| Let's focus on the provided illustration and the associated visuals on the "Semantic Models" dashboard page. <br> <br> The Analysis Services engine (for semantic models) collects different trace logs events to the same OperationId. The illustration shows, which kind of trace logs will be collected during the lifecycle of a query or refresh operation. |
|6.|The first part of the illustration shows that some trace logs events will be already collected before the operation ends. These information doesn't contain all the different key metrics of an operation like memory consumption or CPU time, however it allows us to visualize on this Real-Time Dashboard the count of currently running queries and refreshes, which haven't been finished yet. |
|7.| At the end of each query or refresh operation, another trace log event entry will be collected by the Workspace Monitoring feature, which is the "ExecutionMetrics". This event contains a summary of all key metrics of an operation. However this is only available once the Operations has finished with status "succeeded" or "failed". |

>**Example A**: A semantic model **query** within a Power BI report takes in average around 1.5 minutes. To be able to identify this operation as soon as possible, this dashboard visualizes the "Running Queries" metric. This metric shows, how many different query operations have been started on this monitored workspace within the selected time windows, but now finished yet.

>**Example B**: A semantic model **refresh** takes in average around 2 hours. To be able to identify this operation as soon as possible, this dashboard visualizes the "Started Refreshes" metric. This metric shows, how many different refresh operations have been started on this monitored workspace within the selected time windows, but now finished yet.

**Important to know:** If we would visualize only the insights extracted from the "ExecutionMetrics" only, we wouldn't see any information about a given operation until it has finished.

<br>

#### Active Queries | Semantic Models

Let's continue with the **SM | Active Queries** dashboard page, which helpful especially in situations, where you need to understand, which queries are running currently against the semantic models within the monitored workspace.

![Screenshot](/monitoring/workspace-monitoring-dashboards/media/documentation/rti/fwm_rti_ap_3.png)

|Step|Description|
|---|---|
|Info|We recommend to filter this dashboard page for a small time window like: last _15 min_, _30 min_, _1-2 hours_. |
|9.|All the highlighted visuals show you, who and which query executes currently on the workspace. <br> With the table query (row) will be highlighted with blue color, if the "CurrentStatus" is "Started"|

>**Note:** Please be aware that one Power BI report page can contain multiple visuals. Each visual crafts its own DAX query behind the scenes. <br> The metrics _# Running Queries_ show the number of each unique running DAX and MDX queries which have been called by the user or by a process (like subscription).
...
|Step|Description|
|---|---|
|10.|Once you expanded a row within the table, you can see the raw DAX query, which have been called against the semantic model. |
|11.|**Review** the value of the "EventText" attribute/column. This value is the raw text, which you also can use/call e.g. the DAX editor in Power BI Desktop.|
|12.|Let's assume that you identified already a semantic model where a user/users calling a heavy visual (DAX query). You would like to take action to analyze and optimize the semantic model further. <br> **Explore** three options, which can help you to start an in-depth investigation|


#### Users | Semantic Models

The next dashboard page is the **SM | Users**, which consolidates all the semantic model operations (query and refreshes) by the _Executing User_ perspective. 

>**Info**: Within the SemanticModelLogs table, there are two different user related column. It might occur that a user schedules a semantic model refresh, will will be executed automatically by the service, however this operation is associated to the the user, who owns the semantic model. 
For such scenarios, the _Executing User_ determines the associted user for refreshes, but also defines the user who executes a query ("User opens the Power BI report page, which has 4 visuals -> 4 query operations will be logged.")
...
|Info |In many cases, when we troubleshoot a semantic model, we would like to understand the user behaviors of the user. |
...
|Step|Description|
|---|---|
|17.|The table "Top 50 Failed Operations sorted by CPU Time desc" shows all the recently failed operations with the key metrics by _Executing User_, _Semantic Model_ with link, _OperationId_.|
...
|Step|Description|
|---|---|
|18.|Overview of count of unique refreshes.|
