...
function Remove-AllSoftDeletedArtifacts {
    [CmdletBinding()]
    ...
    $confirmationPromptResponse = Read-Host "Are you sure that you want to permanently delete ALL of your soft-deleted artifacts? Type YES (case-sensitive) to confirm" 
    if ($confirmationPromptResponse -cne "YES") {
        Write-Error "Confirmation not received. Skipping deletion of soft-deleted artifacts. Please try again."
        return
    }
    ...
    try {
        foreach ($wsId in $workspaceIds) {
            ...
            $uri = "https://api.fabric.microsoft.com/v1/workspaces/$wsId/recoverableItems?include=DefaultIdentity"
            do {
                ...
                if ($continuationToken) {
                    $uri = "https://api.fabric.microsoft.com/v1/workspaces/$wsId/recoverableItems?include=DefaultIdentity&continuationToken=$([System.Uri]::EscapeDataString($continuationToken))"
                }
            ...
            foreach ($artifact in $workspaceItems) {
                ...
                while ($true) {
                    ...
                    try {
                        ...
                        Invoke-RestMethod -Method DELETE -Uri "https://api.fabric.microsoft.com/v1/workspaces/$wsId/recoverableItems/$($artifact.id)?hardDelete=True" -Headers $h
