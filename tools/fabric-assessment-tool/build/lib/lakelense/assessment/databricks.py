...
@dataclass
class DatabricksUnityCatalogs:
    unity_catalogs: List[DatabricksUnityCatalog]
...
@dataclass
class DatabricksDatabases:
    """Collection of databases in a Databricks workspace."""

    databases: List[DatabricksDatabase]
...
class DatabricksAssessment:
    ...
    unity_catalogs: Optional[DatabricksUnityCatalogs] = None
    ...
    def get_summary(self) -> dict:
        ...
        summary["counts"]["clusters"] = len(
            self.clusters.clusters
        )
        summary["counts"]["sql_warehouses"] = len(
            self.sql_warehouses.sql_warehouses
        )
        ...
        total_databases = sum(
            len(catalog.databases.databases)
        ...
        total_tables = sum(
            len(schema.tables)
            for catalog in self.catalogs.catalogs
            for schema in catalog.databases.databases
        )
        summary["counts"]["total_tables"] = total_tables

        # Add unity_catalogs count if present
        if self.unity_catalogs is not None:
            summary["counts"]["unity_catalogs"] = len(self.unity_catalogs.unity_catalogs)
        return summary