...
<#
    Behavior tests for Get-FabricOperationsAgent.
    Verifies the OperationsAgents list endpoint + method, default enrichment
...
BeforeAll {
    ...
    Mock -ModuleName MicrosoftFabricMgmt Invoke-FabricAPIRequest {
        ...
        @([pscustomobject]@{ id = 'oa-1'; displayName = 'My Agent'; type = 'OperationsAgent'; workspaceId = 'ws-1'; capacityId = 'cap-1' })
    }
}
...
Describe 'Get-FabricOperationsAgent' -Tag 'UnitTests' {

    It 'calls GET on the OperationsAgents endpoint' {
        $null = Get-FabricOperationsAgent -WorkspaceId 'ws-1'
    ...
    It 'enriches with WorkspaceName, CapacityName and type by default (originals preserved)' {
        $r = Get-FabricOperationsAgent -WorkspaceId 'ws-1'
        $r[0].WorkspaceName         | Should -Be 'WS-Name'
        $r[0].CapacityName          | Should -Be 'Cap-Name'
        $r[0].PSObject.TypeNames[0] | Should -Be 'MicrosoftFabric.OperationsAgent'
        $r[0].displayName           | Should -Be 'My Agent'
    }

    It '-Raw returns the untouched response (no added names, no type)' {
        $r = Get-FabricOperationsAgent -WorkspaceId 'ws-1' -Raw
        ...
        $r[0].PSObject.TypeNames[0]    | Should -Not -Be 'MicrosoftFabric.OperationsAgent'
    }
}
