...
<#
    Behavior tests for Get-FabricDigitalTwinBuilder.
...
BeforeAll {
    ...
    Mock -ModuleName MicrosoftFabricMgmt Invoke-FabricAPIRequest {
        ...
        @([pscustomobject]@{ id = 'dtb-1'; displayName = 'My DTB'; type = 'DigitalTwinBuilder'; workspaceId = 'ws-1' })
    }
}
...
Describe 'Get-FabricDigitalTwinBuilder' -Tag 'UnitTests' {

    It 'calls GET on the workspace digitaltwinbuilders endpoint' {
        $null = Get-FabricDigitalTwinBuilder -WorkspaceId 'ws-1'
        $global:__capUri    | Should -Be 'https://api.fabric.microsoft.com/v1/workspaces/ws-1/digitaltwinbuilders'
        $global:__capMethod | Should -Be 'Get'
    }

    It 'enriches with WorkspaceName, CapacityName and PSTypeName by default (originals preserved)' {
        $r = Get-FabricDigitalTwinBuilder -WorkspaceId 'ws-1'
        $r[0].WorkspaceName         | Should -Be 'WS-Name'
        $r[0].CapacityName          | Should -Be 'Cap-Name'
        $r[0].PSObject.TypeNames[0] | Should -Be 'MicrosoftFabric.DigitalTwinBuilder'
        $r[0].id                    | Should -Be 'dtb-1'
        $r[0].workspaceId           | Should -Be 'ws-1'
    }

    It '-Raw returns the untouched response (no added names, no type)' {
        $r = Get-FabricDigitalTwinBuilder -WorkspaceId 'ws-1' -Raw
        ...
        $r[0].PSObject.TypeNames[0]    | Should -Not -Be 'MicrosoftFabric.DigitalTwinBuilder'
    }
}
