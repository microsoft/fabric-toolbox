...
<#
    Behavior tests for New-FabricSQLDatabase.
...
Describe 'New-FabricSQLDatabase' -Tag 'UnitTests' {
    ...
    It 'calls POST on the workspace sqlDatabases endpoint' {
        $null = New-FabricSQLDatabase -WorkspaceId 'ws-1' -SQLDatabaseName 'MyItem' -Confirm:$false
    ...
    It '-WhatIf makes no API call' {
        ...
        $null = New-FabricSQLDatabase -WorkspaceId 'ws-1' -SQLDatabaseName 'MyItem' -WhatIf
        $global:__capUri | Should -BeNullOrEmpty
    }

    It 'returns the created object' {
        $r = New-FabricSQLDatabase -WorkspaceId 'ws-1' -SQLDatabaseName 'MyItem' -Confirm:$false
