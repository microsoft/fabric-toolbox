...
<#
    Behavior tests for Get-FabricGraphModel.
...
BeforeAll {
    ...
    Mock -ModuleName MicrosoftFabricMgmt Invoke-FabricAPIRequest {
        ...
        @([pscustomobject]@{ id = 'gm-1'; displayName = 'My Graph Model'; type = 'GraphModel'; workspaceId = 'ws-1' })
    }
}
...
Describe 'Get-FabricGraphModel' -Tag 'UnitTests' {

    It 'calls GET on the workspace GraphModels endpoint' {
        $null = Get-FabricGraphModel -WorkspaceId 'ws-1'
    ...
    It 'enriches with WorkspaceName, CapacityName and PSTypeName by default (originals preserved)' {
        $r = Get-FabricGraphModel -WorkspaceId 'ws-1'
        $r[0].WorkspaceName         | Should -Be 'WS-Name'
        $r[0].CapacityName          | Should -Be 'Cap-Name'
        $r[0].PSObject.TypeNames[0] | Should -Be 'MicrosoftFabric.GraphModel'
        $r[0].id                    | Should -Be 'gm-1'
        $r[0].workspaceId           | Should -Be 'ws-1'
    }

    It '-Raw returns the untouched response (no added names, no type)' {
        $r = Get-FabricGraphModel -WorkspaceId 'ws-1' -Raw
        ...
        $r[0].PSObject.TypeNames[0]    | Should -Not -Be 'MicrosoftFabric.GraphModel'
    }
}
