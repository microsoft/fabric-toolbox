...
Describe "New-FabricSemanticModel request body" -Tag "UnitTests" {
    ...
    BeforeAll {
        ...
        Mock -CommandName Get-FileDefinitionPart -ModuleName $ModuleName {
            return @{ parts = @(@{ path = 'model.bim'; payload = 'abc123'; payloadType = 'InlineBase64' }) }
        }
        Mock -CommandName Invoke-FabricAPIRequest -ModuleName $ModuleName {
            return [PSCustomObject]@{ id = 'semanticmodel-1'; displayName = 'My Model' }
        }
    }

    It "Includes folderId in the request body when -FolderId is supplied" {
        New-FabricSemanticModel -WorkspaceId 'ws-1' -SemanticModelName 'My Model' -SemanticModelPathDefinition 'C:\fake\def' -FolderId 'folder-123' -Confirm:$false
    ...
    It "Omits folderId from the request body when -FolderId is not supplied" {
        New-FabricSemanticModel -WorkspaceId 'ws-1' -SemanticModelName 'My Model' -SemanticModelPathDefinition 'C:\fake\def' -Confirm:$false
        Should -Invoke -CommandName Invoke-FabricAPIRequest -ModuleName $ModuleName -Times 1 -Exactly -ParameterFilter {
            $null -eq ($Body | ConvertFrom-Json).folderId
        }
    }

    It "Targets the semanticModels endpoint for the supplied workspace" {
        New-FabricSemanticModel -WorkspaceId 'ws-1' -SemanticModelName 'My Model' -SemanticModelPathDefinition 'C:\fake\def' -Confirm:$false
        Should -Invoke -CommandName Invoke-FabricAPIRequest -ModuleName $ModuleName -Times 1 -Exactly -ParameterFilter {
            $BaseURI -like '*/workspaces/ws-1/semanticModels'
        }
    }
}
