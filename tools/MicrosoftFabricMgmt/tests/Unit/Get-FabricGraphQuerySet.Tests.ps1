...
<#
    Behavior tests for Get-FabricGraphQuerySet.
...
BeforeAll {
    ...
    Mock -ModuleName MicrosoftFabricMgmt Invoke-FabricAPIRequest {
        ...
        @([pscustomobject]@{ id = 'gqs-1'; displayName = 'My Query Set'; type = 'GraphQuerySet'; workspaceId = 'ws-1' })
    }
}
...
Describe 'Get-FabricGraphQuerySet' -Tag 'UnitTests' {

    It 'calls GET on the workspace GraphQuerySets endpoint' {
        $null = Get-FabricGraphQuerySet -WorkspaceId 'ws-1'
        $global:__capUri    | Should -Be 'https://api.fabric.microsoft.com/v1/workspaces/ws-1/GraphQuerySets'
        $global:__capMethod | Should -Be 'Get'
    }

    It 'enriches with WorkspaceName, CapacityName and PSTypeName by default (originals preserved)' {
        $r = Get-FabricGraphQuerySet -WorkspaceId 'ws-1'
        $r[0].WorkspaceName         | Should -Be 'WS-Name'
        $r[0].CapacityName          | Should -Be 'Cap-Name'
        $r[0].PSObject.TypeNames[0] | Should -Be 'MicrosoftFabric.GraphQuerySet'
        $r[0].id                    | Should -Be 'gqs-1'
        $r[0].workspaceId           | Should -Be 'ws-1'
    }

    It '-Raw returns the untouched response (no added names, no type)' {
        $r = Get-FabricGraphQuerySet -WorkspaceId 'ws-1' -Raw
        ...
        $r[0].PSObject.TypeNames[0]    | Should -Not -Be 'MicrosoftFabric.GraphQuerySet'
    }
}
