...
<#
    Behavior tests for Get-FabricDataflow.
...
BeforeAll {
    ...
    Mock -ModuleName MicrosoftFabricMgmt Invoke-FabricAPIRequest {
        ...
        @([pscustomobject]@{ id = 'df-1'; displayName = 'My Dataflow'; type = 'Dataflow'; workspaceId = 'ws-1' })
    }
}
...
Describe 'Get-FabricDataflow' -Tag 'UnitTests' {

    It 'calls GET on the workspace dataflows endpoint' {
        $null = Get-FabricDataflow -WorkspaceId 'ws-1'
        $global:__capUri    | Should -Be 'https://api.fabric.microsoft.com/v1/workspaces/ws-1/dataflows'
        $global:__capMethod | Should -Be 'Get'
    }

    It 'enriches with WorkspaceName, CapacityName and PSTypeName by default (originals preserved)' {
        $r = Get-FabricDataflow -WorkspaceId 'ws-1'
        $r[0].WorkspaceName         | Should -Be 'WS-Name'
        $r[0].CapacityName          | Should -Be 'Cap-Name'
        $r[0].PSObject.TypeNames[0] | Should -Be 'MicrosoftFabric.Dataflow'
        $r[0].id                    | Should -Be 'df-1'
        $r[0].workspaceId           | Should -Be 'ws-1'
    }

    It '-Raw returns the untouched response (no added names, no type)' {
        $r = Get-FabricDataflow -WorkspaceId 'ws-1' -Raw
        ...
        $r[0].PSObject.TypeNames[0]    | Should -Not -Be 'MicrosoftFabric.Dataflow'
    }
}
