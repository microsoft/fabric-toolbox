...
<#
    Behavior tests for Get-FabricAnomalyDetector.
...
BeforeAll {
    ...
    Mock -ModuleName MicrosoftFabricMgmt Invoke-FabricAPIRequest {
        ...
        @([pscustomobject]@{ id = 'ad-1'; displayName = 'My Detector'; type = 'AnomalyDetector'; workspaceId = 'ws-1' })
    }
}
...
Describe 'Get-FabricAnomalyDetector' -Tag 'UnitTests' {

    It 'calls GET on the workspace anomalydetectors endpoint' {
        $null = Get-FabricAnomalyDetector -WorkspaceId 'ws-1'
    ...
    It 'enriches with WorkspaceName, CapacityName and PSTypeName by default (originals preserved)' {
        $r = Get-FabricAnomalyDetector -WorkspaceId 'ws-1'
        $r[0].WorkspaceName         | Should -Be 'WS-Name'
        $r[0].CapacityName          | Should -Be 'Cap-Name'
        $r[0].PSObject.TypeNames[0] | Should -Be 'MicrosoftFabric.AnomalyDetector'
        $r[0].id                    | Should -Be 'ad-1'
        $r[0].workspaceId           | Should -Be 'ws-1'
    }

    It '-Raw returns the untouched response (no added names, no type)' {
        $r = Get-FabricAnomalyDetector -WorkspaceId 'ws-1' -Raw
        ...
        $r[0].PSObject.TypeNames[0]    | Should -Not -Be 'MicrosoftFabric.AnomalyDetector'
    }
}
