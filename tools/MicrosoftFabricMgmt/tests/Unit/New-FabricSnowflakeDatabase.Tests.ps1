...
<#
    Behavior tests for New-FabricSnowflakeDatabase.
...
Describe 'New-FabricSnowflakeDatabase' -Tag 'UnitTests' {

    It 'calls POST on the workspace snowflakeDatabases endpoint' {
        $null = New-FabricSnowflakeDatabase -WorkspaceId 'ws-1' -SnowflakeDatabaseName 'MyItem' -Confirm:$false
    ...
    It '-WhatIf makes no API call' {
        ...
        $null = New-FabricSnowflakeDatabase -WorkspaceId 'ws-1' -SnowflakeDatabaseName 'MyItem' -WhatIf
        $global:__capUri | Should -BeNullOrEmpty
    }

    It 'returns the created object' {
        $r = New-FabricSnowflakeDatabase -WorkspaceId 'ws-1' -SnowflakeDatabaseName 'MyItem' -Confirm:$false
