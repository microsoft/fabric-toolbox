...
<#
    Behavior tests for Get-FabricDigitalTwinBuilderFlow.
...
BeforeAll {
    ...
    Mock -ModuleName MicrosoftFabricMgmt Invoke-FabricAPIRequest {
        ...
        @([pscustomobject]@{ id = 'dtbf-1'; displayName = 'My Flow'; type = 'DigitalTwinBuilderFlow'; workspaceId = 'ws-1' })
    }
}
...
Describe 'Get-FabricDigitalTwinBuilderFlow' -Tag 'UnitTests' {

    It 'calls GET on the workspace DigitalTwinBuilderFlows endpoint' {
        $null = Get-FabricDigitalTwinBuilderFlow -WorkspaceId 'ws-1'
    ...
    It 'enriches with WorkspaceName, CapacityName and PSTypeName by default (originals preserved)' {
        $r = Get-FabricDigitalTwinBuilderFlow -WorkspaceId 'ws-1'
        $r[0].WorkspaceName         | Should -Be 'WS-Name'
        $r[0].CapacityName          | Should -Be 'Cap-Name'
        $r[0].PSObject.TypeNames[0] | Should -Be 'MicrosoftFabric.DigitalTwinBuilderFlow'
        $r[0].id                    | Should -Be 'dtbf-1'
        $r[0].workspaceId           | Should -Be 'ws-1'
    }

    It '-Raw returns the untouched response (no added names, no type)' {
        $r = Get-FabricDigitalTwinBuilderFlow -WorkspaceId 'ws-1' -Raw
        ...
        $r[0].PSObject.TypeNames[0]    | Should -Not -Be 'MicrosoftFabric.DigitalTwinBuilderFlow'
    }
}
