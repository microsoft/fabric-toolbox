...
<#
    Behavior tests for Get-FabricAdminDataflow (list path).
...
BeforeAll {
    ...
    Mock -ModuleName MicrosoftFabricMgmt Invoke-FabricAPIRequest {
        ...
        @([pscustomobject]@{ objectId = 'df-1'; name = 'Sales Dataflow'; workspaceId = 'ws-1' })
    }
}
...
Describe 'Get-FabricAdminDataflow' -Tag 'UnitTests' {

    It 'calls GET on the admin dataflows endpoint' {
        $null = Get-FabricAdminDataflow
        $global:__capUri    | Should -Be 'https://api.powerbi.com/v1.0/myorg/admin/dataflows'
        $global:__capMethod | Should -Be 'Get'
    }

    It 'enriches with WorkspaceName, CapacityName and type by default (originals preserved)' {
        $r = Get-FabricAdminDataflow
        $r[0].WorkspaceName         | Should -Be 'WS-Name'
        $r[0].CapacityName          | Should -Be 'Cap-Name'
        $r[0].PSObject.TypeNames[0] | Should -Be 'MicrosoftFabric.AdminDataflow'
        $r[0].objectId              | Should -Be 'df-1'
        $r[0].workspaceId           | Should -Be 'ws-1'
    }

    It '-Raw returns the untouched response (no added names, no type)' {
        $r = Get-FabricAdminDataflow -Raw
        $r[0].PSObject.Properties.Name | Should -Not -Contain 'WorkspaceName'
        $r[0].PSObject.TypeNames[0]    | Should -Not -Be 'MicrosoftFabric.AdminDataflow'
    }

    It 'scopes to a workspace group when -WorkspaceId is supplied' {
        $null = Get-FabricAdminDataflow -WorkspaceId 'ws-9'
        $global:__capUri | Should -Be 'https://api.powerbi.com/v1.0/myorg/admin/groups/ws-9/dataflows'
    }
}
