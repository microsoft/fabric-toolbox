...
<#
    Behavior tests for New-FabricCosmosDBDatabase.
...
Describe 'New-FabricCosmosDBDatabase' -Tag 'UnitTests' {

    It 'calls POST on the workspace cosmosDbDatabases endpoint' {
        $null = New-FabricCosmosDBDatabase -WorkspaceId 'ws-1' -CosmosDBDatabaseName 'MyItem' -Confirm:$false
    ...
    It '-WhatIf makes no API call' {
        ...
        $null = New-FabricCosmosDBDatabase -WorkspaceId 'ws-1' -CosmosDBDatabaseName 'MyItem' -WhatIf
        $global:__capUri | Should -BeNullOrEmpty
    }

    It 'returns the created object' {
        $r = New-FabricCosmosDBDatabase -WorkspaceId 'ws-1' -CosmosDBDatabaseName 'MyItem' -Confirm:$false
