<#
.SYNOPSIS
    Retrieves the definition of a Graph Query Set item from a specific workspace in Microsoft Fabric.

.DESCRIPTION
    This function fetches the Graph Query Set item's content or metadata from a workspace.
    It supports both synchronous and asynchronous operations, with detailed logging and error handling.

.PARAMETER WorkspaceId
    (Mandatory) The unique identifier of the workspace from which the Graph Query Set definition is to be retrieved.
...
.EXAMPLE
    ...
    Retrieves the definition of the Graph Query Set item with ID 67890 from the workspace with ID 12345.
...
#>
function Get-FabricGraphQuerySetDefinition {
    [CmdletBinding()]
    param (
    ...
    try {
        ...
        $segments = @('workspaces', $WorkspaceId, 'GraphQuerySets', $GraphQuerySetId, 'getDefinition')
