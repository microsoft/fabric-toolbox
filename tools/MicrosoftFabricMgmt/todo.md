...
Baseline QA-gate failures: 323. FINAL build,test: **7978 passed / 1 failed** (the 1 = pre-existing `Clear-FabricNameCache` cross-test cache-isolation flake; passes 10/0 in isolation, was failing in the 323 baseline). ScriptAnalyzer/Author/missing-test gates all cleared.
Commits: e1bfa5c (enrich/-Raw/tooling), 72ae570 (analyzer/authors/process-blocks), 4c6d4d9 (Get-* tests + -Segments fix), 3291e6b (non-Get-* tests).
- [ ] OPTIONAL: fix the Clear-FabricNameCache flake (make it robust to cache entries left by other tests / clear in BeforeEach) for a fully-green `build,test`.
